...
public class ConfigurationServer extends UnicastRemoteObject implements ConfigurationServices {
	...
	public Model openModel(String name, RemoteSession remoteSession)
			throws ServerException, ModelException {
		...
		return models.initModel(name, session.getConfiguration());
	}
	...
	public ConfigurationResponse findConfiguration(Configuration configuration, RemoteSession remoteSession)
		...
		return handler.findCompleteConfiguration(configuration);
	}
	...
	public ConfigurationResponse computeConfigurationState(Configuration configuration, RemoteSession remoteSession) throws RemoteException, ServerException {
		logger.config("Received configuration request from client ("
				+ remoteSession + ")");
		...
		ConfigurationHandler handler = session.getConfiguration();
		return handler.getConsequences(configuration);
	}
